...
/** A class that represent a Multiply Force operation
 *
 * @param quantity this indicate how much the force going to be multiplied
 * @param nextOp the next operation that will be executed, by default is a NoOperation object
 */
class MultiplyForce(quantity: Int, nextOp: IOperation = new NoOperation) extends IOperation {

  /** Method to multiply force of each card of the target
   *
